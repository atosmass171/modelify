...
stripe.api_key = "sk_test_51PftYHD29AQ4tzG9PqZX96C3iUgtcjG34yKJ9Uo3LjD6ddUqETf2SQbScHws9tCSjJcA08fKeXEbggs9fCaA2nZo006YxuV4SP"
...
app.config['GOOGLE_CLIENT_ID'] = '433782134884-1oi4iibl36ab1te398co3seh2esp1kfv.apps.googleusercontent.com'
app.config['GOOGLE_CLIENT_SECRET'] = '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'
...
def pausar_bot(bot_id):
    ...
    conn.close()
    return redirect(url_for("meus_bots"))
...
def ativar_bot(bot_id):
    ...
    conn.close()
    return redirect(url_for("/api/meus_bots"))
...
def excluir_bot(bot_id):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT product_uuid FROM bots WHERE id=? AND user_email=?", (bot_id, user_email))
    row = cursor.fetchone()
    product_uuid = row["product_uuid"] if row else None
    conn.execute("DELETE FROM bots WHERE id=? AND user_email=?", (bot_id, user_email))
    conn.commit()
    conn.close()
    if product_uuid:
        # Sempre volta pra página do produto/bots daquele produto (mesmo se não restar nenhum bot)
        return redirect(url_for("bot", product_uuid=product_uuid))
    # Se não conseguiu descobrir o product_uuid, tenta voltar pra página anterior
...
def editar_bot(bot_id):
    ...
    if request.method == "POST":
        ...
        confirmation_keyword = request.form.get('confirmation_keyword')
        confirmation_message = request.form.get('confirmation_message')
        ...
        cursor.execute("""
            UPDATE bots
               SET bot_name=?, bot_token=?, welcome_message=?, confirmation_keyword=?, confirmation_message=?, group_id=?, photo_filename=?
             WHERE id=? AND user_email=?
        """, (bot_name, bot_token, welcome_message, confirmation_keyword, confirmation_message, group_id, foto, bot_id, user_email))
        conn.commit()
        conn.close()
        return redirect(url_for("meus_bots"))
    conn.close()
    return render_template("editar_bot.html", bot=dict(bot))
...
def saque():
    ...
    if request.method == 'POST':
        print("DEBUG FORM DATA:", request.form)  # <-- debug
        ...
        if not valor_str:
            return render_template(
                'saque.html',
                user=session['user'],
                receita_gerada=get_receita_gerada(user_email),
        ...
        print("SALVANDO SAQUE:", user_email, nome_completo, chave_pix, data_pedido, status, banco, tipo_chave, valor)
...
def membros(product_uuid):
    ...
    print(f"product[5]: '{product[5]}'")
    print(f"Comparação: {product[5] == user_id}")

    if product[5].strip().lower() != user_id.strip().lower():
    ...
    product_details = {
        ...
        'preco': product[2],
        'imagem': product[3],
        'quantidade_vendas': product[4],
        'email_dono': product[5],
        'descricao': product[6],
        'url_checkout': product[7],
        'url_flow': product[8],
        'uuid': product[9]
    }
